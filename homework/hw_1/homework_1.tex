...
\centerline{
  {\large
    \textbf{This assignment is due by 11:59 p.m. on Sunday February 15,
      2026.}
  }
}
...
and $\mathbf{I}$ is the \href{https://en.wikipedia.org/wiki/Moment_of_inertia#Inertia_tensor}{moment of inertia tensor}, which is defined as:
...
making some changes to your code from the in-class assignment!
...
comet are integrated for a single orbit using a fixed time step
($\simeq 1$ and $\simeq 5.2$ years for the planet and comet, respectively) and,
...
\item  Group 1:  Allison, Brennom, Justin, Tessa
\vspace{-2mm}

\item  Group 2:  Grace, McKenna, Meir, Tom

\vspace{-2mm}
\item  Group 3:  John, Sydney, Cassidy
...
%\vspace{-1mm}
  ...
  branch to
  GitHub as well.  If there is a fourth group member (not all groups
  have one!) make an additional branch, make some simple edit to the
  code (to, e.g., modify some existing print statements), and issue a
  PR for that.
