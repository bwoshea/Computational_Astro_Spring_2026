...
\centerline{ {\large
\textbf{This assignment is due by 11:59 p.m. on Friday March 13,
  2026.} } }
...
part of your homework grade).  To that end, please put your code in
text files, \textbf{not} Jupyter notebooks!  Also, please run
\href{https://pypi.org/project/pylint/}{PyLint} and make sure that
your code doesn't have any obvious issues (i.e., don't spend a lot of
time fixing warnings about style issues that don't indicate bugs). 
Plots should be named
...
PDF document of a similar name (e.g., \texttt{ANSWERS.pdf}).  Please
also report how much time that you spend on each problem!
...
line.  Also plot a second lot that shows $\epsilon_{\Delta x} \propto
\Delta t^2$ for comparison.}  Note: if you don't see a convergence rate of
...
this circumstance?  Hint: make sure that you're consistent with units!  If the gravitational
constant is in CGS, positions and densities must also be in CGS. Note
also that the
routine in \texttt{star\_array\_generator.py} returns densities in CGS units.
...
solution that you would expect\footnote{Hint: make a log-log plot of the
magnitude of the potential vs. distance -- if you're calculating it
correctly you should get a line with a slope of $\pm 1$ depending on
which side of the star you're on.}.  You may also want to make an image of
a slice of the potential array\footnote{Hint: you may wish to take the log of
the absolute magnitude of a slice of the array to be able to see
dynamic range!}.
For the default resolution, how many iterations does it take for your solution to
...
\item Congratulations!  You've written a (very slow) gravity solver.  If you wanted
  to turn this into a simple simulation of the dynamical evolution of a cluster
  of stars, how might you do that?  (To be clear, ``dynamical
  evolution'' in this case means following the evolution of the stars'
  positions and velocities over time.)  You don't need to write the actual
...
Hints: when working on this problem, think carefully about the inner and
outer boundary conditions.  What should those look like, in terms of
their values and how you implement that?  Where do you expect the
temperature to be a maximum, and where should it be a minimum?
