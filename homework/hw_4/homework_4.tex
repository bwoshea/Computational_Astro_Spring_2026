...
\centerline{ {\large
\textbf{This assignment is due by 11:59 p.m. on Friday May 1,
  2026.} } }
...
traditional but difficult to justify in the 21st century, in the
opinion of your admittedly theory-centric professor.}:
...
assumptions about Bayesian priors, the amount of data, the properties
of errors in data,
...
your starting point\footnote{Note that we're using the MCMC code from
  class for this assignment because it's easy to understand and modify, but if you use MCMC for your own research
you should use a tool like
\href{https://emcee.readthedocs.io/en/stable/}{emcee} instead -- it
uses a more modern and efficient MCMC algorithms, can be run in
parallel, and has lots of useful plotting tools associated with it!}, conduct the following experiments and report the
...
\item Finally, repeat experiment \#1 keeping the number of bins
  constant but modifying the nature of
  the errors for each bin.  In the \texttt{luminosity\_function()}
  routine, look for the string ``UNCOMMENT AND ADD YOUR CODE HERE''
  and read the comment block above it, which talks about the nature of
  galaxy surveys.  Try setting up error estimates that approximate
  different types of galaxy surveys and note how those errors impact
  both the best-fit model as well as the behavior of the posterior
  distributions.

\end{enumerate}

In the document describing your answers, reflect on the sum
...
of observed data, the magnitude and nature of the errors in observed data, your
assumptions about the prior distribution, and the measured best-fit
parameters and posterior distribution.  I'm looking for qualitative
comments here -- what are the takeaways that you can generalize to
interpreting papers or talks that deal with this type of model
...
(using, e.g., the \href{https://science.nasa.gov/mission/kepler/}{Kepler mission}),
...
\href{https://exoplanet.eu/catalog/51_peg_b--12/}{51-Pegasi} (in the data
