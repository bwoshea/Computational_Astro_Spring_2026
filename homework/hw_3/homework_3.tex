...
\centerline{ {\large
\textbf{This assignment is due by 11:59 p.m. on Friday April 10,
  2026.} } }
...
Salesman Problem} is a classic and challenging problem in computer science where the
focus is on optimization 
The problem is as follows: Imagine there is
a salesman who has to travel to $N_C$ cities.  The order is
unimportant, as long as they only visit each city once on each trip,
and finishes where they started.  The salesman wants to keep the
...
\item Assuming you can test one model per microsecond 
  per core using ICER's High Performance Computing Center (which has approximately
  $7 \times 10^4$ CPU cores across several clusters), how long would it take to test
  every single combination for $N_C = 30$ to get the provably best solution, and how does that compare
  to how long it takes to get a reasonably good solution using your implementation?

\end{enumerate}

Note that you should do many trials per choice of $N_S$ to get a sense
of the typical
behavior!  Regarding trials, it's very important to recall that Python's \href{https://docs.python.org/3/library/random.html}{random} library
uses the
\href{https://en.wikipedia.org/wiki/Mersenne_Twister}{Mersenne
  Twister} algorithm, which is a \href{https://en.wikipedia.org/wiki/Pseudorandom_number_generator}{pseudorandom number generator} -- it uses an algorithm that
generates a sequence of numbers that are \textit{statistically random},
but which are repeatable given the same initial number. In other
words, the sequence of numbers has properties that approximate a truly
random sequence of numbers, but the order of numbers is \textit{deterministic}.  This means
that you need to change the initial ``random seed'' number to get a
different sequence of ensuing numbers!

%\textbf{A suggestion:} You will parallelize the code that you write here in the
%next section of this homework assignment, so consider ways that you can
%write your code that will facilitate doing so!  (For example, use
%NumPy as much as possible rather than lists!)

\newpage
\noindent {\large\textbf{Part 3:}}  In this section you're going to
speed up the Python code that you wrote in Part 2 of this homework
problem using the parallelism techniques you learned in class.  To
that end, do the following (and document timing and other progress
along the way with tables, figures, and/or text):

\begin{enumerate}
  \item Establish a performance baseline by timing the single-core
    version of your code for a variety of different numbers of
    cities (from just a few cities up to 100 cities).  Also use timers
    (from the Python
    \href{https://docs.python.org/3/library/time.html}{time library}
    -- e.g. \texttt{time.perf\_counter()}) to identify the costly part(s) of your
    code, as an opportunity for future optimization. How does the cost
    change?  Hint: run multiple trials and change the random seed in
    the random number generator to get a sense of performance
    variability as a function of system load (i.e., what other
    processes are running on the CPU core) and the number of
    iterations at a given cost.

  \item Examine your code and look for serial ``hot spots'' -- i.e., places
    where the code is taking significant fractions of the computing
    time in its current serial (single CPU core) form.  It's useful to do this for different numbers of cities --
    as you change that, you'll see that due to the algorithmic
    complexity of the problem different parts will change in cost
    while others remain the same!
    
  \item Try to speed up the hot spots by making simple fixes -- for
    example, try to find places where you can replace lists with NumPy
    arrays, loops with NumPy operations, etc. Test those fixes with
    additional timing tests and document what you did and how it
    changed the code's speed. Note that if you did a good job
    writing your code you may not see much improvement here!

  \item Try accelerating your serial code using
    \href{https://numba.pydata.org/}{Numba} decorators.  How much does
    this accelerate your code for different numbers of cities for a
    given random seed (i.e., a given number of iterations per number
    of cities)?  If you try Numba's
    \href{https://numba.readthedocs.io/en/stable/user/parallel.html}{parallelization}
    options, what happens?  (Hint: think about what Numba does to
    parallelize this problem -- how easily can it be parallelized as
    you've currently formulated it?)
   
  \item Now accelerate the code by using the Python
    \href{https://docs.python.org/3/library/multiprocessing.html}{multiprocessing}
    library (do this separately from your Numba effort).  Focus your parallelism efforts on the hot spots you
    identified.  Document the speedup compared to the serial case.
    Hint: how would you parallelize this optimization problem?  Which
    parts of the problem do you need to distribute to different CPU cores, and how would you
    do that?  Also, what do you need to do with regard to the random
    seed(s) you use for optimization?  (Think about what would happen
    if every CPU core uses the same random seed - what would you
    expect to happen?)

  \end{enumerate}

 Submit the various versions of the code that you've written in
 clearly-named files.  Also, make sure to write a narrative explaining
 the things that you've done to speed up your code and the results
 that you see.  For the problem you're trying to accelerate, is there
 an optimal number of CPU cores where you get the best speedup?  How
 does that vary as a function of the problem size and optimization technique?

\vspace{2mm}
 \noindent
\textbf{Hint:} You're going to need Python \textit{partial functions}
for the Multiprocessing component of this option.  Check out
\href{https://www.pythontutorial.net/python-basics/python-partial-functions/}{this
  tutorial}
on partial functions.  Here's an example of how to use a partial
function:
...
Alternately, you can use \texttt{mutliprocessing.Pool().starmap},
which is similar to the function taught in class
(\texttt{multiprocessing.Pool().map}) but handles functions with
multiple arguments. This option is useful if you don't want every
process to run your desired function with the same arguments (as will
happen when using a partial function). 


 
\newpage
\noindent {\large\textbf{Part 4:}}  ``Stellar population synthesis''
...
%\newpage
\vspace{5mm}
\noindent {\large\textbf{Part 5:}}  It has been hypothesized that the
...
directory \texttt{part\_5\_files}) containing a
...
different (non-zero) ages.  How might this affect your approach and/or
