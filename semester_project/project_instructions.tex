...
\item Implement a 1D finite volume hydro solver using the Piecewise
  Linear update (Section 8.2.2 of Zingale).  Also implement the
...
\item Make sure to document all of your tests, including convergence
  ...
  arguments and automatically output plots comparing to reference solutions!
...
(though it will also require you to add an additional ghost
zone!).
...
sizes and numbers of grids, and plot the timing for both a
\href{https://hpc-wiki.info/hpc/Scaling}{strong and weak scaling} test 
...
using a batch job (either using a \href{https://docs.icer.msu.edu/SLURM_overview/}{Slurm script} or
\href{https://docs.icer.msu.edu/Interactive_Job/}{interactive job} to ensure that nobody else is running on the same
...
brief ($\simeq 1$~paragraph) proposal describing which project
...
\item \textbf{Project presentations and code demonstrations, due 7:45
    a.m. on Friday 5/1/2026 -- immediately before this course's final
    exam session}. Your final
